"""
更新低碳城市试点名单：将济南市从第二批改为第三批
"""

import pandas as pd

print("=" * 100)
print("更新低碳城市试点名单：济南市 2012/第二批 → 2017/第三批")
print("=" * 100)
...
print(f"第二批城市数: {len(batch_2['cities'])} (移除济南市: 19 → 18)")
print(f"第三批城市数: {len(batch_3['cities'])} (添加济南市: 41 → 42)")
...
print(f"\n[变更说明]")
print(f"  济南市: 第二批(2012) → 第三批(2017)")

print(f"\n第二批城市列表（共18个）:")
batch2_df = df[df['批次'] == '第二批']
for idx, row in batch2_df.iterrows():
    print(f"  - {row['城市名']}")

print(f"\n第三批城市列表（共42个，部分预览）:")
batch3_df = df[df['批次'] == '第三批']
for idx, row in batch3_df.iterrows():
    print(f"  - {row['城市名']}")
...
jinan = df[df['城市名'] == '济南市']
if len(jinan) > 0:
    jinan_info = jinan.iloc[0]
    print(f"\n[验证] 济南市信息:")
    print(f"  批次: {jinan_info['批次']}")
    print(f"  政策实施年: {jinan_info['政策实施年']}")
    if jinan_info['批次'] == '第三批' and jinan_info['政策实施年'] == 2017:
        print(f"  [OK] 济南市已成功调整到第三批(2017)")
    else:
        print(f"  [ERROR] 济南市调整失败！")
else:
    print(f"\n[ERROR] 未找到济南市！")
...
print("更新完成!")
print("=" * 100)
print(f"\n第二批: 19个城市 → 18个城市 (-1 济南市)")
print(f"第三批: 41个城市 → 42个城市 (+1 济南市)")
print(f"总计: 保持142个城市")
