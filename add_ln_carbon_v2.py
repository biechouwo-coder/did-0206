...
# 检查碳排放量列是否存在
if '碳排放量_吨' not in df.columns:
    print("\n[错误] 数据集中不存在 '碳排放量_吨' 列")
    print(f"可用列: {list(df.columns)}")
    exit(1)

# 如果ln_碳排放量_吨已存在，询问是否覆盖
if 'ln_碳排放量_吨' in df.columns:
    print(f"\n[注意] 变量 'ln_碳排放量_吨' 已存在，将被覆盖")

# 统计信息
print(f"\n原始碳排放量统计:")
print(df['碳排放量_吨'].describe())

# 检查缺失值和零值
missing = df['碳排放量_吨'].isna().sum()
zero_values = (df['碳排放量_吨'] == 0).sum()
negative_values = (df['碳排放量_吨'] < 0).sum()
...
# 创建ln_碳排放量_吨变量
print(f"\n生成新变量: ln_碳排放量_吨")
df['ln_碳排放量_吨'] = np.log(df['碳排放量_吨'])

# 统计新变量
print(f"\n新变量统计:")
print(df['ln_碳排放量_吨'].describe())

# 检查是否有无穷值（由零值或负值取对数产生）
inf_values = np.isinf(df['ln_碳排放量_吨']).sum()
print(f"\n无穷值数量: {inf_values}")
