...
quantile_df = pd.DataFrame({
    ...
    '值': [f"{df[col_name].quantile(q):.4f}" for q in quantiles]
})
...
trend_df = pd.DataFrame({
    ...
    '同比增长(%)': yearly_change.values
})
trend_df['同比增长(%)'] = trend_df['同比增长(%)'].apply(lambda x: f"{x:+.2f}%" if not np.isnan(x) else "N/A")
...
print(f"\n总体趋势:")
print(f"  {first_year}年: {first_val:.4f}")
print(f"  {last_year}年: {last_val:.4f}")
print(f"  总变化: {total_change:+.4f} ({total_change_pct:+.2f}%)")
...
def classify_financial(x):
    if pd.isna(x):
        return '缺失'
    elif x < mean_val - 0.5 * std_val:
        return '低水平'
    elif x < mean_val + 0.5 * std_val:
        return '中等水平'
    else:
...
df['金融发展等级'] = df[col_name].apply(classify_financial)
financial_dist = df['金融发展等级'].value_counts()
financial_pct = df['金融发展等级'].value_counts(normalize=True) * 100

financial_df = pd.DataFrame({
    '金融发展等级': financial_dist.index,
    '观测数': financial_dist.values,
    '比例(%)': financial_pct.values
})
...
print("\n【13. 经济学解释与意义】")
print("-" * 80)
print(f"金融发展水平均值: {df[col_name].mean():.4f}")
print(f"金融发展水平中位数: {df[col_name].median():.4f}")
print(f"变异系数: {df[col_name].std() / df[col_name].mean():.4f}")

print("\n金融发展水平的含义:")
print("• 反映一个地区金融体系的发达程度")
print("• 通常用金融机构存贷款余额/GDP等指标衡量")
print("• 高水平意味着金融资源配置效率高")
print("• 与经济发展、投资、创新密切相关")

print("\n金融发展水平的区域特征:")
print("• 东部沿海地区金融发展水平较高")
print("• 直辖市和省会城市较为突出")
print("• 随着金融改革，各地区水平逐步提升")

print("\n在DID模型中的作用:")
print("• 作为重要控制变量")
print("• 控制金融发展对碳排放的间接影响")
print("• 提高模型估计的准确性")
